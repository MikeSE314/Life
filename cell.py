...
class LivingCell(object):
    ...
    def give_to_neighbors(self):
        return(self.color, [(self.xCoord - 1, self.yCoord - 1), (self.xCoord,
            self.yCoord - 1), (self.xCoord + 1, self.yCoord - 1), (
            self.xCoord - 1, self.yCoord), (self.xCoord + 1, self.yCoord),
            (self.xCoord - 1, self.yCoord + 1), (self.xCoord, self.yCoord + 1),
    ...
    def draw(self, dirtyList, window_object, width, xScale = 1, yScale = 1):
        dirtyList.append(pygame.draw.rect(window_object, self.color, ((
            self.xCoord * width + self.xCoord + 1) * xScale, (self.yCoord *
            width + self.yCoord + 1) * yScale, width * xScale, width *
            yScale)))
